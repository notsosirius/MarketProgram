...
public class SalerLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Saler mbr = service.findByEmail(email);
		
		//step3: 执行跳转
		if(mbr != null){
			if(mbr.getPwd().equals(pwd)){
				if(mbr.getCategory()!=null) {
					//登录成功
					//在会话中记录当前登录的会员信息
					request.getSession().setAttribute("curr_mbr", mbr);
					request.getSession().setAttribute("curr_mbr_cate",2);
					SessionInfo sessionInfo=new SessionInfo();
					sessionInfo.setUser_cate(2);
					...
					//显示商品列表
					response.sendRedirect(request.getContextPath() + "/saler/products");
				}else {
					request.setAttribute("msg", "你的身份有待管理员验证，请验证后登录");
					request.getRequestDispatcher("/saler_login.jsp").forward(request, response);
				}
