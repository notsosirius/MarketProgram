 ...
 */
@WebServlet("/AdministratorReportFormServlet")
public class AdministratorReportFormServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdministratorReportFormServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("showall".equals(action)) {
			...
			if(catelist != null) {
				for(Category cate:catelist) {
					//Double cateWhole = Double.parseDouble(cateService.getRevenueByName(cate.getName()));
					Double cateWhole = cateService.getRevenueByName(cate.getName());
					Integer cateId = cateService.getIdByName(cate.getName());
					Revenue revenue = new Revenue(cateId, cate.getName(), cateWhole);
					revenueList.add(revenue);
					page.setItems(revenueList);
				}
		...
		request.getRequestDispatcher("/administrator/sale/report.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//List<Category> catelist = cateService.findAllSub();
		List<Revenue> revenueList=new ArrayList<Revenue>();
		Double cateWhole = cateService.getRevenueByCondition(cate, startTime, endTime);
		//Long cateWhole = cateService.getRevenueByCondition(cate);
		Integer cateId = cateService.getIdByName(cate);
		Revenue revenue = new Revenue(cateId, cate, cateWhole);
		revenueList.add(revenue);
		page.setItems(revenueList);
		...
		if(revenueList!=null && revenueList.size()>0) {
			...
			if(startTimeString.length()==0||endTimeString.length()==0) {
				...
				String path = "<script>alert('请输入查询日期！');"
					    + "window.location.href=ctx+'/AdministratorReportFormServlet?action=showall'</script>";
