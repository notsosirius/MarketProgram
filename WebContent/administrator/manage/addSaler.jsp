		...
		<div class="row" style="padding: 20px 0px;">
			...
			<div class="col-xs-2">
				...
				<jsp:include page="/administrator/menu.jsp">
					<jsp:param value="addSaler" name="tag" />
				</jsp:include>
			</div>
			<!-- /左边 -->
			...
			<div class="col-xs-10">
				<div class="panel">
					...
					<div class="col-xs-4" id="search">
						<form action="${ctx}/searchSaler" method="post">
						...
						<tbody>
							<c:forEach items="${page.items}" var="pro">
								<tr>
									...
									<c:if test="${!empty pro.category}">
										<td class="text-center">${pro.category}</td>
									</c:if>
									<c:if test="${empty pro.category}">
										<td>
											<input type="text" name="category_id" value="" id="category_id"/>
										</td>
										<td>
											<a proId="${pro.id}" ctx="${ctx}" id="transfer" onClick="func()">确认</a>
											<script type="text/javascript">
												function func(){
													var category=$("#category_id").val();
													var proId=$("#transfer").attr("proId");
													var ctx=$("#transfer").attr("ctx");
													location.href=ctx+"/ConfirmCategoryServlet?id="+proId+"&category_id="+category;
												}
												
												//$("#transfer").attr("href",ctx+"/ConfirmCategoryServlet?id="proId+"&category_id="+category); 
											</script>
										</td>
									</c:if><!-- 不知道怎么设计设置类别 类别最好还是显示的文字而不是编号 --><!-- 实现了 -->
								</tr>
							</c:forEach>
